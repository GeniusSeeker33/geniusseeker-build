...
app.post("/api/employer/login", (req, res) => {
    ...
    db_1.db.prepare("INSERT INTO employer_sessions (id, email, token, created_at, expires_at) VALUES (?,?,?,?,?)").run(`sess_${(0, uuid_1.v4)()}`, email, token, createdAt, expiresAt);
...
function requireEmployer(req, res, next) {
    ...
    const sess = db_1.db.prepare("SELECT * FROM employer_sessions WHERE token=? AND expires_at > ?").get(token, nowISO());
    if (!sess)
...
app.post("/api/profile/upsert", (req, res) => {
    ...
    if (!hederaAccountId)
        return res.status(400).json({ error: "hederaAccountId required" });
    const id = `profile_${hederaAccountId}`;
    const existing = db_1.db.prepare("SELECT * FROM profiles WHERE id=?").get(id);
    const name = displayName?.trim() || null;
    if (existing) {
        db_1.db.prepare("UPDATE profiles SET display_name=? WHERE id=?").run(name ?? existing.display_name, id);
    }
    else {
...
app.post("/api/profile/update", (req, res) => {
    const { hederaAccountId, displayName, bio, skills, avatarUrl, portfolioUrl, linkedinUrl, githubUrl, openToWork, resumeText, resumePdfUrl } = req.body || {};
    if (!hederaAccountId)
        return res.status(400).json({ error: "hederaAccountId required" });
    const id = `profile_${hederaAccountId}`;
    const existing = db_1.db.prepare("SELECT * FROM profiles WHERE id=?").get(id);
    if (!existing) {
        db_1.db.prepare("INSERT INTO profiles (id, hedera_account_id, display_name, created_at) VALUES (?,?,?,?)").run(id, hederaAccountId, displayName?.trim() || null, nowISO());
    }
    db_1.db.prepare(`
    UPDATE profiles SET
    ...
    WHERE id = ?
  `).run(displayName?.trim() || null, bio?.trim() || null, skills ? JSON.stringify(skills) : null, avatarUrl?.trim() || null, portfolioUrl?.trim() || null, linkedinUrl?.trim() || null, githubUrl?.trim() || null, openToWork || null, resumeText?.trim() || null, resumePdfUrl?.trim() || null, id);
    const profile = db_1.db.prepare("SELECT * FROM profiles WHERE id=?").get(id);
...
app.get("/api/profile/:accountId", (req, res) => {
    ...
    if (!profile)
        return res.json({ profile: null, credentials: [] });
    if (profile.skills) {
        try {
    ...
    const token = auth.replace("Bearer ", "").trim();
    const sess = token
        ? db_1.db.prepare("SELECT * FROM employer_sessions WHERE token=? AND expires_at > ?").get(token, nowISO())
        : null;
    if (!sess)
        delete profile.hedera_account_id;
...
app.get("/api/candidates", requireEmployer, (req, res) => {
    ...
    const session = db_1.db.prepare("SELECT * FROM employer_sessions WHERE token=?").get(sessionToken);
    if (session) {
        const employer = db_1.db.prepare("SELECT tier, status FROM employer_profiles WHERE contact_email=?").get(session.email);
    ...
    const { steam, open_to_work } = req.query;
    let query = `
    SELECT p.*,
  ...
  `;
    const params = [];
    if (open_to_work) {
        query += " AND p.open_to_work = ?";
        params.push(open_to_work);
    }
    query += " ORDER BY p.created_at DESC LIMIT 100";
    const candidates = db_1.db.prepare(query).all(...params).map(c => {
        if (c.skills) {
            try {
...
app.post("/api/credentials/verify", async (req, res) => {
    const parsed = validators_1.VerifyCredentialSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const { hederaAccountId, credentialType, metadata } = parsed.data;
    const createdAt = nowISO();
    const id = `cred_${(0, uuid_1.v4)()}`;
    let hederaTokenId;
    let hederaSerial;
    let txId;
    ...
    VALUES (?,?,?,?,?,?,?,?)
  `).run(id, hederaAccountId, credentialType, JSON.stringify(metadata), hederaTokenId ?? null, hederaSerial ?? null, txId ?? null, createdAt);
...
app.post("/api/badges/issue", async (req, res) => {
    const parsed = validators_1.IssueBadgeSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const { hederaAccountId, displayName, badge } = parsed.data;
    const createdAt = nowISO();
    const quizVersion = (req.body?.quizVersion && String(req.body.quizVersion)) ||
        (req.body?.badge?.quizVersion && String(req.body.badge.quizVersion)) ||
        "STEAM_V1";
    // upsert profile
    const profileId = `profile_${hederaAccountId}`;
    const existingProfile = db_1.db.prepare("SELECT * FROM profiles WHERE id=?").get(profileId);
    if (!existingProfile) {
    ...
    (id, hedera_account_id, credential_type, metadata_json, hedera_token_id, hedera_serial, tx_id, created_at)
    VALUES (?,?,?,?,?,?,?,?)
  `).run(id, hederaAccountId, "STEAM_BADGE", JSON.stringify(payload), hederaTokenId ?? null, hederaSerial ?? null, txId ?? null, createdAt);
...
app.post("/api/value/log", (req, res) => {
    try {
        ...
        VALUES (?,?,?,?,?,?,?,?)
      `).run(`ve_${(0, uuid_1.v4)()}`, String(actor), String(eventType), String(amount), String(currency), reference ?? null, metadata ? JSON.stringify(metadata) : null, ts);
        }
        catch (e) {
...
app.get("/api/value/events", (_req, res) => {
    try {
        const events = db_1.db.prepare("SELECT * FROM value_events ORDER BY created_at DESC LIMIT 200").all();
        res.json({ events });
    }
    catch (e) {
...
app.post("/api/employer/apply", (req, res) => {
    const { companyName, website, contactName, contactEmail, contactTitle, companySize, industry, ein, 
    // attestations
    postsSalary, hasDeiPolicy, diversePanels, structuredFeedback, flexibleWork, tracksEquity, pipelinePrograms, payEquityAudit, deiPolicyUrl, equityReportUrl, additionalNotes } = req.body || {};
    if (!companyName || !contactName || !contactEmail) {
        return res.status(400).json({ error: "companyName, contactName, and contactEmail are required" });
    }
    // Check for duplicate
    const existing = db_1.db.prepare("SELECT id FROM employer_profiles WHERE contact_email=?").get(contactEmail);
    if (existing) {
        return res.status(409).json({ error: "An application with this email already exists" });
    }
    const empId = "emp_" + (0, uuid_1.v4)();
    const now = nowISO();
    db_1.db.prepare(`
  ...
  `).run(empId, companyName, website || null, contactName, contactEmail, contactTitle || null, companySize || null, industry || null, ein || null, now);
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
  `).run("att_" + (0, uuid_1.v4)(), empId, postsSalary ? 1 : 0, hasDeiPolicy ? 1 : 0, diversePanels ? 1 : 0, structuredFeedback ? 1 : 0, flexibleWork ? 1 : 0, tracksEquity ? 1 : 0, pipelinePrograms ? 1 : 0, payEquityAudit ? 1 : 0, deiPolicyUrl || null, equityReportUrl || null, additionalNotes || null, now);
...
app.get("/api/employer/status/:email", (req, res) => {
    const emp = db_1.db.prepare("SELECT id, company_name, tier, status, created_at, reviewed_at FROM employer_profiles WHERE contact_email=?").get(req.params.email);
    if (!emp)
...
app.post("/api/employer/review", (req, res) => {
    const { employerId, applicationId, reviewerHedera, stage, accurateDescription, clearCommunication, respectfulProcess, salaryMatched, wouldApplyAgain, comments } = req.body || {};
    ...
    const existing = db_1.db.prepare("SELECT id FROM employer_reviews WHERE application_id=? AND stage=?").get(applicationId, stage || "application");
    if (existing)
        return res.status(409).json({ error: "Review already submitted for this application" });
    db_1.db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
  `).run("rev_" + (0, uuid_1.v4)(), employerId, applicationId, reviewerHedera, stage || "application", accurateDescription || null, clearCommunication || null, respectfulProcess || null, salaryMatched || null, wouldApplyAgain || null, comments || null, nowISO());
    // Recompute employer score (avg of all 5-star ratings)
    const reviews = db_1.db.prepare(`
...
app.get("/api/employer/reviews/:employerId", requireEmployer, (req, res) => {
    const reviews = db_1.db.prepare("SELECT * FROM employer_reviews WHERE employer_id=? ORDER BY created_at DESC").all(req.params.employerId);
...
app.post("/api/recruiter/apply", (req, res) => {
    const { fullName, email, phone, linkedinUrl, steamFields, experience, industries, whyJoin, referralSource } = req.body || {};
    if (!fullName || !email) {
        return res.status(400).json({ error: "fullName and email are required" });
    }
    // Check for duplicate
    const existing = db_1.db.prepare("SELECT id FROM recruiter_applications WHERE email=?").get(email);
    if (existing) {
        return res.status(409).json({ error: "An application with this email already exists." });
    }
    const id = "rec_" + (0, uuid_1.v4)();
    const now = nowISO();
    db_1.db.prepare(`
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,'pending',?)
  `).run(id, fullName, email, phone || null, linkedinUrl || null, Array.isArray(steamFields) ? steamFields.join(",") : (steamFields || null), experience || null, industries || null, whyJoin || null, referralSource || null, now);
...
app.get("/api/recruiter/applications", requireEmployer, (req, res) => {
    const apps = db_1.db.prepare("SELECT * FROM recruiter_applications ORDER BY created_at DESC").all();
    res.json({ ok: true, applications: apps });
});
// POST /api/recruiter/applications/:id/approve
app.post("/api/recruiter/applications/:id/approve", requireEmployer, (req, res) => {
    const { adminNotes } = req.body || {};
    db_1.db.prepare("UPDATE recruiter_applications SET status='approved', admin_notes=? WHERE id=?").run(adminNotes || null, req.params.id);
    res.json({ ok: true, status: "approved" });
});
// POST /api/recruiter/applications/:id/reject
app.post("/api/recruiter/applications/:id/reject", requireEmployer, (req, res) => {
    const { adminNotes } = req.body || {};
    db_1.db.prepare("UPDATE recruiter_applications SET status='rejected', admin_notes=? WHERE id=?").run(adminNotes || null, req.params.id);
...
function calcPlacementFee(rate, contractType) {
    const num = parseFloat(rate.replace(/[^0-9.]/g, "")) || 0;
    if (contractType === "hourly")
        return (num * 160 * 0.10).toFixed(2); // 1 month @ 40hr/wk
    return (num * 0.10).toFixed(2);
}
// POST /api/offers — employer makes an offer to a candidate
app.post("/api/offers", requireEmployer, (req, res) => {
    const { employerId, employerEmail, employerCompany, candidateHedera, candidateName, roleTitle, contractType, rate, currency, startDate, scope, milestonesJson } = req.body || {};
    ...
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,'pending',?,?,?)
  `).run(id, employerId, employerEmail || "", employerCompany || "", candidateHedera, candidateName || null, roleTitle, contractType, rate, currency || "USD", startDate || null, scope || null, milestonesJson ? JSON.stringify(milestonesJson) : null, fee, now, now);
    res.json({ ok: true, id, placementFee: fee });
});
// GET /api/offers/employer/:employerId — employer sees their sent offers
app.get("/api/offers/employer/:employerId", requireEmployer, (req, res) => {
    const offers = db_1.db.prepare("SELECT * FROM offers WHERE employer_id=? ORDER BY created_at DESC").all(req.params.employerId);
    res.json({ ok: true, offers });
});
// GET /api/offers/candidate/:hederaId — candidate sees received offers
app.get("/api/offers/candidate/:hederaId", (req, res) => {
    const offers = db_1.db.prepare("SELECT * FROM offers WHERE candidate_hedera=? ORDER BY created_at DESC").all(req.params.hederaId);
    res.json({ ok: true, offers });
});
// POST /api/offers/:id/accept — candidate accepts
app.post("/api/offers/:id/accept", (req, res) => {
    const { hederaId } = req.body || {};
    const offer = db_1.db.prepare("SELECT * FROM offers WHERE id=?").get(req.params.id);
    if (!offer)
        return res.status(404).json({ error: "Offer not found" });
    if (offer.candidate_hedera !== hederaId)
        return res.status(403).json({ error: "Not your offer" });
    if (offer.status !== "pending" && offer.status !== "countered") {
        return res.status(409).json({ error: `Offer is already ${offer.status}` });
    }
    db_1.db.prepare("UPDATE offers SET status='accepted', updated_at=? WHERE id=?").run(nowISO(), req.params.id);
...
app.post("/api/offers/:id/decline", (req, res) => {
    ...
    if (offer.candidate_hedera !== hederaId)
        return res.status(403).json({ error: "Not your offer" });
    db_1.db.prepare("UPDATE offers SET status='declined', updated_at=? WHERE id=?").run(nowISO(), req.params.id);
...
app.post("/api/offers/:id/agree", requireEmployer, (req, res) => {
    db_1.db.prepare("UPDATE offers SET status='agreed', updated_at=? WHERE id=?")
...
app.post("/api/offers/:id/create-deel-contract", requireEmployer, async (req, res) => {
    const offer = db_1.db.prepare("SELECT * FROM offers WHERE id=?").get(req.params.id);
    if (!offer)
        return res.status(404).json({ error: "Offer not found" });
    if (offer.status !== "agreed")
        return res.status(409).json({ error: "Offer must be agreed before creating contract" });
    const DEEL_API_KEY = process.env.DEEL_API_KEY;
    const now = nowISO();
    // SANDBOX MODE — simulate contract creation if no Deel key
    if (!DEEL_API_KEY) {
        const fakeDeelId = "deel_sandbox_" + (0, uuid_1.v4)().slice(0, 8);
        db_1.db.prepare("UPDATE offers SET status='contracted', deel_contract_id=?, updated_at=? WHERE id=?")
      ...
      VALUES (?,?,?,'active',?,?,?,?)
    `).run("con_" + (0, uuid_1.v4)(), offer.id, fakeDeelId, offer.rate, offer.currency || "USD", now, now);
        db_1.db.prepare("INSERT INTO deel_sync_log (id,event_type,entity_id,payload_json,status,created_at) VALUES (?,?,?,?,?,?)")
            .run("log_" + (0, uuid_1.v4)(), "contract_created_sandbox", offer.id, JSON.stringify({ mode: "sandbox", deelId: fakeDeelId }), "ok", now);
    ...
    try {
        ...
        const deelData = await deelRes.json();
        if (!deelRes.ok) {
            db_1.db.prepare("INSERT INTO deel_sync_log (id,event_type,entity_id,payload_json,status,created_at) VALUES (?,?,?,?,?,?)")
                .run("log_" + (0, uuid_1.v4)(), "contract_create_error", offer.id, JSON.stringify(deelData), "error", now);
      ...
      INSERT INTO contracts (id, offer_id, deel_contract_id, deel_status, total_value, currency, created_at, updated_at)
      VALUES (?,?,?,'active',?,?,?,?)
    `).run("con_" + (0, uuid_1.v4)(), offer.id, deelContractId, offer.rate, offer.currency || "USD", now, now);
        db_1.db.prepare("INSERT INTO deel_sync_log (id,event_type,entity_id,payload_json,status,created_at) VALUES (?,?,?,?,?,?)")
            .run("log_" + (0, uuid_1.v4)(), "contract_created_live", offer.id, JSON.stringify(deelData), "ok", now);
...
app.post("/api/webhooks/deel", (req, res) => {
    const event = req.body;
    const now = nowISO();
    db_1.db.prepare("INSERT INTO deel_sync_log (id,event_type,entity_id,payload_json,status,created_at) VALUES (?,?,?,?,?,?)")
        .run("log_" + (0, uuid_1.v4)(), event?.type || "unknown", event?.data?.contract_id || null, JSON.stringify(event), "ok", now);
    // Handle key events
    if (event?.type === "contract.activated" || event?.type === "contract.signed") {
        const deelContractId = event?.data?.id;
        if (deelContractId) {
            db_1.db.prepare("UPDATE contracts SET deel_status='active', signed_at=? WHERE deel_contract_id=?").run(now, deelContractId);
            db_1.db.prepare("UPDATE offers SET status='contracted', updated_at=? WHERE deel_contract_id=?").run(now, deelContractId);
        }
    }
    if (event?.type === "payment.completed") {
        const deelContractId = event?.data?.contract_id;
        if (deelContractId) {
            db_1.db.prepare("UPDATE contracts SET deel_status='payment_completed', updated_at=? WHERE deel_contract_id=?").run(now, deelContractId);
        }
    }
    if (event?.type === "contract.terminated") {
        const deelContractId = event?.data?.id;
        if (deelContractId) {
            db_1.db.prepare("UPDATE contracts SET deel_status='terminated', terminated_at=?, updated_at=? WHERE deel_contract_id=?")
                .run(now, now, deelContractId);
            db_1.db.prepare("UPDATE offers SET status='completed', updated_at=? WHERE deel_contract_id=?").run(now, deelContractId);
        }
    }
    res.json({ received: true });
});
// GET /api/contracts/:offerId — get contract status for an offer
app.get("/api/contracts/:offerId", (req, res) => {
    const contract = db_1.db.prepare("SELECT * FROM contracts WHERE offer_id=?").get(req.params.offerId);
    if (!contract)
        return res.status(404).json({ error: "No contract found for this offer" });
...
app.post("/api/apply", async (req, res) => {
    try {
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?)
      `).run(id, job_title, company || null, name, email, phone || null, portfolio || null, hedera_id || null, cover_note || null, createdAt);
        }
        catch (e) {
            console.error("DB write failed (continuing):", e);
        }
...
app.get("/api/applications", (_req, res) => {
    try {
        const applications = db_1.db.prepare("SELECT * FROM applications ORDER BY created_at DESC LIMIT 200").all();
...
app.post("/api/submit-job", async (req, res) => {
    try {
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      `).run(id, company, website || null, title, steam || null, type || null, remote || null, location || null, pay || null, deadline || null, desc || null, email, "pending", createdAt);
        }
        catch (e) {
            console.error("DB write failed (continuing):", e);
        }
...
app.get("/api/job-submissions", (_req, res) => {
    try {
        const jobs = db_1.db.prepare("SELECT * FROM job_submissions ORDER BY created_at DESC LIMIT 200").all();
        res.json({ jobs });
    }
    catch (e) {
        console.error("DB read failed:", e);
...
app.post("/api/job-submissions/:id/approve", (req, res) => {
    try {
        const { id } = req.params;
        const job = db_1.db.prepare("SELECT * FROM job_submissions WHERE id=?").get(id);
        if (!job)
            return res.status(404).json({ error: "Job not found" });
        db_1.db.prepare("UPDATE job_submissions SET status=? WHERE id=?").run("approved", id);
...
app.post("/api/job-submissions/:id/reject", (req, res) => {
    try {
        ...
        if (!job)
            return res.status(404).json({ error: "Job not found" });
        db_1.db.prepare("UPDATE job_submissions SET status=? WHERE id=?").run("rejected", id);
...
app.get("/api/jobs", (_req, res) => {
    try {
        const jobs = db_1.db.prepare("SELECT * FROM job_submissions WHERE status='approved' ORDER BY created_at DESC").all();
        res.json({ jobs });
    }
    catch (e) {
        console.error("DB read failed:", e);
